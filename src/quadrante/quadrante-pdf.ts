...
export interface EventoDoPdf {
  ...
  logo?: string;
  /** a mesma logo, na altura do cabeçalho — ver `quadrante-images.ts` */
  logoDoCabecalho?: string;
  capa?: string;
...
export function cabecalho(evento: EventoDoPdf): string {
  const paleta = paletaDoEvento(evento.colors);
  const marca = evento.logoDoCabecalho ?? evento.logo;
  const logo = marca
    ? `<img src="${marca}" style="height: 7mm; max-width: 28mm; object-fit: contain; margin-right: 3mm;" />`
    : '';
